...
const newReviewHtml = `<!-- Screen: Exam Review Flow -->
        <div id="view-exam-review" class="quiz-view">
            <!-- Header -->
            ...
            <div class="view-content" style="padding-top: 16px; padding-bottom: 120px;">
                <!-- Summary Card -->
                <div class="format-card" style="display: flex; flex-direction: column; align-items: stretch; padding: 20px; margin-bottom: 24px; background: white; border-radius: 16px; border: 1.5px solid rgba(15,23,42,0.04); box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02);">
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; border-bottom: 1px solid #f1f5f9; padding-bottom: 16px;">
                        ...
                        <span id="review-exam-date" style="font-size: 14px; font-weight: 700; color: #0f172a; text-align: right;">--</span>
                    </div>
                    <div style="display: flex; justify-content: space-between; align-items: center; margin-bottom: 16px; border-bottom: 1px solid #f1f5f9; padding-bottom: 16px;">
                ...
                <div id="review-question-container" style="background: white; border-radius: 16px; padding: 24px; border: 1.5px solid rgba(15,23,42,0.04); margin-bottom: 24px; box-shadow: 0 4px 6px -1px rgba(0,0,0,0.02);">
                    ...
                    <div class="quiz-content-area" style="padding: 0; min-height: auto;">
                        <h2 class="question-text" id="review-q-text">
                            <!-- Question Text -->
                        </h2>
                        
                        <div style="width: 500px; max-width: 100%;">
                            <div class="answers-grid" id="review-options-container">
                    ...
                    <!-- Explanation -->
                    <div style="margin-top: 24px; background: #f8fafc; border-radius: 12px; padding: 20px; border: 1px solid rgba(15,23,42,0.04);">
        ...
        <script>
            // Utility to generate a massive bank of questions for Mock Exam #6
            function generateMassiveBank(count) {
                const bank = [];
                for(let i = 1; i <= count; i++) {
                    const isCorrectUser = Math.random() > 0.3; // 70% chance they got it right
                    const correctIdx = Math.floor(Math.random() * 4);
                    const userIdx = isCorrectUser ? correctIdx : (correctIdx + 1) % 4;

                    bank.push({
                        text: \`Sergeant Exam Question #\${i}: According to Department Policy 400.1, what is the mandatory reporting time for a critical incident?\`,
                        options: [
                            { text: "Immediately or as soon as physically possible", isCorrect: correctIdx === 0 },
                            { text: "Within 24 hours of the incident", isCorrect: correctIdx === 1 },
                            { text: "Before the end of the officer's shift", isCorrect: correctIdx === 2 },
                            { text: "Within 48 hours to Internal Affairs", isCorrect: correctIdx === 3 }
                        ],
                        userSelectedIndex: userIdx,
                        explanation: \`Policy 400.1 specifically states that all critical incidents must be reported through the chain of command as outlined in the correct option. This is heavily tested on the Sergeant Exam.\`
                    });
                }
                return bank;
            }
            ...
            function openExamReview(element) {
                ...
                // Assign subject and bank based on the mock exam tapped
                if (title.includes('#6')) {
                    subject = "Full Sergeant Exam (150 Questions)";
                    currentActiveBank = examBanks.mock6;
                } else if (title.includes('#1')) {
                    subject = "Cardiology Module";
                    currentActiveBank = examBanks.mock1;
                } else if (title.includes('#2')) {
                    subject = "Pulmonology Module";
                ...
                // Update Header with Title + Subject
                document.getElementById('review-exam-title').innerText = \`\${title}: \${subject}\`;
            ...
            function renderReviewNav() {
                ...
                // For 150 questions, rendering 150 buttons takes a tiny bit of time, but DOM can handle it in a horizontal flex layout.
                currentActiveBank.forEach((q, idx) => {
                    ...
                    btn.onclick = () => {
                        ...
                        // Scroll nav to center the clicked button
            ...
            function changeReviewQuestion(dir) {
                ...
                if (newIdx >= 0 && newIdx < currentActiveBank.length) {
                    ...
                    const navContainer = document.getElementById('review-nav-container');
                    const targetBtn = navContainer.children[newIdx];
                    if(targetBtn) {
...
if (html.match(reviewBlockRegex)) {
    ...
    console.log("Successfully rebuilt the Exam Review Flow with background fix and 150 questions.");
} else {
